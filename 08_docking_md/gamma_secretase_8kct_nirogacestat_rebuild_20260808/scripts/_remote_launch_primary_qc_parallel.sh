...
patterns = (
    ...
    "run_primary_structure_parallel.py",
...
run_id="$(date -u +%Y%m%dT%H%M%SZ)_primary_qc_parallel_v1"
...
set +e
"$py" scripts/run_primary_structure_parallel.py \
  --manifest "$manifest" \
  --output-root "$out" \
  > "$out/logs/structural_parallel.log" 2>&1
struct_code=$?
write_status structural_parallel finished "$struct_code"
set -e
...
cat > "$runner_json" <<EOF
{
  ...
  "policy": "parallel_structural_then_fixed_energy_then_membrane_then_primary_validation",
