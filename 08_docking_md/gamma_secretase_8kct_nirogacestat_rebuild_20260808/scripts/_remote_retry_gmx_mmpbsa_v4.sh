...
audit="$root/audit/toolchain_install/20260822_gmx_mmpbsa_retry_v4/$stamp"
prior_audit="$root/audit/toolchain_install/20260822_network_retry_v3/20260822T110831Z"
old=/root/autodl-tmp/envs/gmx_mmpbsa_1_6_5_cpu_20260822_v3
fresh=/root/autodl-tmp/envs/gmx_mmpbsa_1_6_5_cpu_20260822_v4
mkdir -p "$audit"
[[ ! -r /proc/2005/stat ]] || { echo 'refusing active prior gmx_MMPBSA installer' >&2; exit 3; }
[[ ! -e "$fresh" ]] || { echo "refusing existing v4 target: $fresh" >&2; exit 4; }
if [[ -f "$prior_audit/gmx_mmpbsa_install.log" ]]; then
  sha256sum "$prior_audit/gmx_mmpbsa_install.log" > "$audit/PRIOR_MPI_BUILD_FAILURE.sha256"
fi
if [[ -e "$old" ]]; then
  archived="${old}.failed_mpi_build_${stamp}"
  [[ ! -e "$archived" ]]
  mv "$old" "$archived"
