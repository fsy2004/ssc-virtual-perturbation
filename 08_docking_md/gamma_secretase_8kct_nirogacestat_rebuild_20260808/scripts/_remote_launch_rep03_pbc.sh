...
log="$runtime/rep03_primary_pbc.log"
pidfile="$runtime/rep03_primary_pbc.pid"
mkdir -p "$runtime"
...
if [[ -e "$pidfile" ]]; then
  ...
  if [[ "$oldpid" =~ ^[0-9]+$ ]] && kill -0 "$oldpid" 2>/dev/null; then
    echo "REFUSE: recorded rep03 PBC PID is still active: $oldpid" >&2
    exit 75
  fi
fi
cd "$root"
...
echo "LAUNCHED rep03 primary PBC pid=$pid log=$log"
