#!/usr/bin/env python3
"""Independently validate one hash-bound O6U MP2 torsion scan point.

Version 2 accepts the start geometry as an explicit validator input.  This
supports v1 scan reports that bound the geometry by SHA256 but did not repeat
its filesystem path in the JSON report.
"""
...
def main() -> int:
    ...
    parser.add_argument("--expected-start-xyz", required=True, type=Path)
    ...
    start_path = args.expected_start_xyz.resolve()
    ...
    bound_canary_hash = scan.get("canary_validation_sha256")
    checks["canary_validation_precondition"] = bound_canary_hash in (None, sha256(canary_path))
    ...
    checks["charge_multiplicity"] = scan.get("charge") == 0 and scan.get("multiplicity") == 1
    ...
    checks["resource_route"] = (
        scan.get("threads") == args.expected_threads
        and scan.get("memory_gib") == 44
        and scan.get("scan_parallelism", 1) == 1
    )
    ...
    checks["start_xyz_hash"] = start_path.is_file() and sha256(start_path) == scan.get("start_xyz_sha256")
    ...
    start_elements, start_coordinates, _ = read_xyz(start_path)
    frame_path = Path(auth["inputs"]["frame342"]["path"])
    frame_elements, frame_coordinates, _ = read_xyz(frame_path)
    checks["atom_identity_order"] = elements == frame_elements == start_elements
    ...
    checks["recomputed_dihedral"] = (
        recomputed_error <= 0.5
        and abs(circular_delta(recomputed_angle, float(scan["final_dihedral_deg"]))) <= 1e-5
    )
    frame_angle = dihedral(frame_coordinates, ids)
    previous_index = args.expected_step_index - (1 if args.expected_step_index > 0 else -1)
    expected_start_angle = frame_angle + previous_index * float(scan.get("grid_spacing_deg", 15.0))
    recomputed_start_angle = dihedral(start_coordinates, ids)
    recomputed_start_error = abs(circular_delta(recomputed_start_angle, expected_start_angle))
    checks["adjacent_start_geometry"] = recomputed_start_error <= 0.5
    ...
    report = {
        ...
        "expected_start_xyz": {"path": str(start_path), "sha256": sha256(start_path)},
        ...
        "recomputed_start_dihedral_deg": recomputed_start_angle,
        "recomputed_start_error_deg": recomputed_start_error,
        "checks": checks,
