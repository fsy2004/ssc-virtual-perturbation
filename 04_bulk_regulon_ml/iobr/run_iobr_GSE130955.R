# auto-generated IOBR deconvolution (module 492) for GSE130955  [SEED 123]
suppressMessages({library(IOBR); library(tidyverse)})
set.seed(123)
eset <- as.matrix(read.csv("./out_05_bulk/stageB/GSE130955_expr_for_deconv.csv", row.names=1, check.names=FALSE))
...
decon <- lapply(methods, function(m)
  tryCatch(deconvo_tme(eset=eset, method=m, arrays=FALSE, perm=1000),
...
write.csv(tme, "./out_05_bulk/stageB/GSE130955_deconv.csv")
